...
# If last entry is today AND it's not a SKIPPED entry, we already sent one
if [ "$LAST_DATE" = "$TODAY" ] && [[ "$LAST_TITLE" != "SKIPPED"* ]]; then
  exit 0
fi
...
# Calculate starting index based on line count (rotation)
LOG_LINES=$(wc -l < "$INQUIRY_LOG" 2>/dev/null || echo 0)
START_IDX=$(( LOG_LINES % ${#FALLBACKS[@]} ))

# Try to find an available question (rotates through all until one passes dedup)
SENT=false
for i in $(seq 0 $((${#FALLBACKS[@]} - 1))); do
  # Calculate actual index (rotate from start position)
  IDX=$(( (START_IDX + i) % ${#FALLBACKS[@]} ))
  ENTRY="${FALLBACKS[$IDX]}"
  ...
  # Check if this topic passes dedup
  ...
  if [ "$SUPPRESSED" = "true" ]; then
    # This topic is in cooldown, try next
    ...
    echo "{\"date\":\"$TODAY\",\"title\":\"SKIPPED\",\"topic\":\"$TOPIC_KEY\",\"reason\":\"$SUPPRESS_REASON\",\"days_remaining\":$DAYS_REMAINING,\"index_tried\":$IDX}" >> "$INQUIRY_LOG"
    continue
  fi
  
  # Topic is available! Send the question.
  ...
  # Log success
  echo "{\"date\":\"$TODAY\",\"title\":\"$TITLE\",\"topic\":\"$TOPIC\",\"index\":$IDX,\"semantic_topic\":\"$TOPIC_KEY\",\"attempts\":$((i+1))}" >> "$INQUIRY_LOG"
  
  # Update legacy tracking file for backward compat
  python3 << PYSCRIPT
import json
import time
...
try:
    ...
    data = {"schema_version": "2.0", "topics": {}, "last_updated": 0}
...
# If no question was available (all in cooldown), log it but don't error
if [ "$SENT" = "false" ]; then
  echo "{\"date\":\"$TODAY\",\"title\":\"BLOCKED\",\"reason\":\"all-topics-in-cooldown\",\"count_skipped\":${#FALLBACKS[@]}}" >> "$INQUIRY_LOG"
fi
